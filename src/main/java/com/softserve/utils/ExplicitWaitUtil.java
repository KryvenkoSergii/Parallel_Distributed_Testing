...
//        this.wait = new WebDriverWait(driver, readProjectProperties.geExplicitWaitDelay());
    ...
    public void setFluentWait() {
        fluentWait = wait.withTimeout(Duration.of(ELEMENT_CONDITION_TIMEOUT, ChronoUnit.SECONDS))
                .pollingEvery(Duration.of(POLLING_INTERVAL, ChronoUnit.SECONDS)).ignoring(WebDriverException.class);
    }
    ...
    public void setAwait() {
        await = await().atMost(CONDITION_MET_TIMEOUT, TimeUnit.SECONDS).pollInterval(POLLING_INTERVAL, TimeUnit.SECONDS);
    }
    ...
    public void elementHasStyleDisplayNone(String selectorXPath) {
        ...
        setImplicitWait(0);
        wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(selectorXPath + "[contains(@style, 'display: none')]")));
        setImplicitWait(5);
    }
