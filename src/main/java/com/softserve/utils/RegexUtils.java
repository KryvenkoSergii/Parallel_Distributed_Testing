...
//            System.out.println("minutes = " + matcher2.group(1) + "; sec = " + matcher2.group(2));
        ...
        final Matcher matcher3 = Pattern.compile("^(\\d+)\\D+(\\d+)\\D+(\\d+)").matcher(time);
        final Matcher matcher4 = Pattern.compile("^(\\d+)\\D+(\\d+)").matcher(time);
        if (matcher3.find()) {
//          System.out.println("hours = " + matcher3.group(1) + "; minutes = " + matcher3.group(2) + "; sec = " + matcher3.group(3));
          timeSec = 3600 * Integer.parseInt(matcher3.group(1)) + 60 * Integer.parseInt(matcher3.group(2))
                  + Integer.parseInt(matcher3.group(3));
      } else if (matcher4.find()) {
//          System.out.println("minutes = " + matcher4.group(1) + "; sec = " + matcher4.group(2));
          timeSec = 60 * Integer.parseInt(matcher4.group(1)) + Integer.parseInt(matcher4.group(2));
      }
...
//        getVideoDuration("  1:01:05 ");
        getTime("1 hours 8 минут 26 секунд");
    }
}